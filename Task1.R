...
# title: "Task 1 - Retail Strategy and Analytics - Quantium Virtual Internship"
# author: "Gifty Cheruvallimalayil"
# date: "24/05/2022"
...
# Lets take a look at each of the data sets provided.
...
# We can see that the date column in the transaction dataset is in an integer format. 
...
# not, let's remove all words with digits and special characters 
# such as '&' from our set of product words. 
...
# Next, we can use `summary()` to check summary statistics such as 
# mean, min and max values for each feature to see if there are any 
# obvious outliers in the data and if there are any nulls in any of the columns 
...
# We can see that there is an increase in purchases in December 
# and a break in late December. Let's zoom in on this.
...
# As the number of rows in `data` is the same as that of `transaction_data`, we can be
...
# `transaction_data` and find rows with matching values in shared columns and then
...
# - Mainstream young singles/couples are 56% less likely 
# to purchase Burger Rings compared to the rest of the population

# - Mainstream young singles/couples are 23% more likely 
# to purchase Tyrrells chips compared to the rest of the population
